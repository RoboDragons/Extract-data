    ...
    while not stop_event.is_set():
        try:
            ...
            if ball_position:
                ...
                # print("len(ballposi)",len(ball_posi))
                ...
                if len(ball_posi) == 2:
                    ...
                    velocity_data.append(vx,vy,speed,frame1)
                    send_data
                    ballpojiPath = os.path.join(path, "ball_velocity_data.csv")
                    if not os.path.isdir(path):
                        os.mkdir(path)
                    if ball_position:
                        df = pd.DataFrame(velocity_data, columns=["vx", "vy", "speed","frame_number"])
                        if not os.path.exists(ballpojiPath):
                            df.to_csv(ballpojiPath, mode='w', header=True, index=False)
                        else:
                            df.to_csv(ballpojiPath, mode='a', header=False, index=False)
